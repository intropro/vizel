                ...
                link: function (scope, element) {
                    ...
                    function mapOptions() {
                        return {
                            ...
                            fills: scope.options.fills ? angular.extend({}, scope.options.fills) : {
                                defaultFill: '#b9b9b9'
                            },
                            data: {}
                        };
                    ...
                    function mapData(dst, data) {
                        angular.forEach(data, function (val) {
                            dst.data[val.location] = val.value;
                        });
...
//                            debugger;
                            ...
                            var w = null;
                            if(!scope.height){
                                h = scope.mapOptions.element.offsetHeight;
                            }
                            if(!scope.width){
                                w = scope.mapOptions.element.offsetWidth;
                            }

                            if(w && h && w > h / 0.6){
                                w = scope.mapOptions.width = h / 0.6;
                            }
                            ...
                            // set legend
                            if (options.legend) {
                                scope.map.legend(options.legend);
                            }
